...
var csvImport = angular.module('ngCsvImport', ['angular-growl']);
angular.module('ngCsvImport').config([
    'growlProvider', 
...
csvImport.directive('ngCsvImport', function(growl) {
	return {
		restrict: 'E',
		transclude: true,
		replace: true,
		...
		link: function(scope, element, attrs) {
			...
			element.on('change', function(onChangeEvent) {
				...
				if (onChangeEvent.target.files[0].size > 1024*1024*5) {
				    growl.error("File size should be less than 5M");
				    return;
				...
				if (scope.filename.substr(scope.filename.length - 3) !== 'csv') {
			        growl.error("Please import csv file");
			        return;
			    }
